...
- Version 11.x — team release. Increment point version each fix/feature (v11.1, v11.2, ...)
- Version bump format: `LegionnAIre v11.X` — increment the point version each fix/feature
- When bumping version, also set `config_version=X` (the point version) in ALL 11 conf files: `legion.conf`, `app/masterLegion.conf`, and 9 profiles in `app/profiles/`
...
- **v10.162**: Updated requirements.txt and install_tools.sh after full dependency audit via commit history review. requirements.txt: removed asyncio (stdlib), PyQt6/qasync (GUI-only), GitPython/pandas (unused in Flask mode), flake8/webdriver-manager (dev-only); added flask, werkzeug, anthropic, google-auth, neotermcolor with correct version ranges; updated SQLAlchemy to >=1.4,<3.0 (2.x compatible), selenium to >=4.9.0, urllib3 to >=2.0.0. install_tools.sh: updated pre-installed tool list to full scope, added urlfinder (Go), neotermcolor pip install (always-ensure), Python requirements auto-install, expanded summary verification list.
- **v10.163**: Full requirements + Docker verification + unit test. requirements.txt now covers BOTH Flask web mode (flask, werkzeug, anthropic, google-auth) AND Qt6 GUI mode (PyQt6, qasync, pandas, GitPython) with correct version ranges. Dockerfile.test verifies a clean Kali install: pulls kalilinux/kali-rolling, pip installs requirements.txt, runs 22 pytest cases (package imports for all 16 declared packages, Flask starts and /api/snapshot responds, Qt6 QApplication offscreen, 71 tool binary checks skipped in Docker). tests/test_requirements.py added to run_tests.sh --unit section with --noconftest flag.
...
- **v10.188**: Project-switch session race — proper architectural fix replaces v10.179 `runCommand` workaround. `WebController._release_outgoing_session()` calls `session.remove()` on the OUTGOING project's database BEFORE switching; called by `createNewProject`, `openExistingProject`, `saveProjectAs`. Eliminates `OperationalError: no such table` non-determinism in test_api_gaps. Two test fixes ride along: (1) `test_api_gaps::test_p13_second_save_open_cycle` no longer unlinks `path2` in `finally:` — defers to atexit via `_files_to_cleanup`, because the SQLAlchemy pool keeps FDs to the file alive and unlinking mid-run causes SQLite to silently create a fresh empty file with no schema on the next pool refill. (2) `test_selenium_terminal::test_plain_dynamic_tab_shows_text` was using a non-existent CSS class `.dynamic-tab-content.active` (real class is `.tab-content`). Verified: 5 consecutive runs of test_api_gaps → 45/45.
- **v10.189**: Tier-ordered `run_tests.sh` + anti-pattern guards. New `tests/test_anti_patterns.py` (3 grep-based guards, 0.07s) blocks: (1) `os.unlink` of `.legion` files outside an atexit cleanup, (2) `killRunningProcesses()` from any non-shutdown route handler, (3) unjustified `time.sleep(>=5)` in tests (legitimate cases tag with `# DETERMINISM-EXEMPT: <reason>`). Both unit and selenium sections now run in tiers — smoke/sanity → fragile → bulk → slow → live. With OLD chronological order, regression in code touched today took 14 min to surface (session_fixes ran last); NEW order surfaces it in <2 min. Tagged 7 existing legitimate long sleeps (process-timeout test, scroll-overflow tests, two-staged-scan settle, etc.).
- **v10.190**: Deleted 6 hollow MagicMock-only test files (1873 lines): `integration/test_SmokeTests` (17 mocks, asserts on mock identity), `integration/test_UIRegressions` (PyQt6-only, crashed in Flask), `features/test_HtmlOutputStorage`, `features/test_TabSwitchingDataIntegrity`, `features/test_NotesSaveFix` (asserts `.commit.assert_called_once`), `features/test_ToolTabOutputPersistence`. Audit confirmed ZERO unique coverage loss — every claimed behaviour is already verified by real `test_api_gaps` and `test_save_open_data` (real DB / real client). KEPT after audit: `test_CoreWorkflows`, `test_CriticalPaths`, `test_ConfigSyntaxValidation` — these have real assertions despite being in the same dirs. Wired 6 of 7 `test_goal*.py` (12 new tests covering Ctrl+B, ANSI preservation, match nav, selection confinement). Skipped: `test_goal3_ansi_ctrlb` has 2 failing tests since v10.50 — likely real ANSI-preservation regression worth separate triage.
- **v10.191**: Phase 3 determinism sweep — replaced ~30 `time.sleep(>=1.0s)` instances with explicit `WebDriverWait` across 5 files (`test_ui_v10b_features`, `test_ui_new_features`, `test_ui_new_features2`, `test_ui_v10d_features`, `test_goal5_notes_formatting`). Each replacement waits for the EXACT condition the next code expects (DOM state, snapshot field, API response). New helpers added per file: `_wait_proc_status` (snapshot poll for status), `_wait_btn_text` (button textContent change), `_wait_proc_in_snapshot` (snapshot poll with predicate), `_wait_dom_row` (row presence/absence), `_wait_host_selected` (L.selectedHostIp). Suite faster AND deterministic: v10b 70s → 54s, new_features 50s → 32s, new_features2 32s → 26s. **AI dropdown gotcha**: `#ai-compare-select` always starts with a placeholder option (`No similar`/`Select host`), then gets replaced atomically after the `/api/ai/history/similar` fetch. Don't wait for `options.length >= 1` — pre-fetch the same API yourself to know the EXPECTED option count, then wait for the dropdown to match exactly.
- **v10.192**: Phase 4 route coverage — new `tests/test_ui_route_coverage.py` (port 5092, 12 tests, 45s) for routes that were API-only or live-only: (1) `/api/processes/custom` end-to-end with `[IP]/[PORT]` substitution + output verification (was modal-open-only); (2) `/api/screenshot/take` with valid/missing-host/missing-port + screenshooter-process-appears-in-snapshot (was live-network-only via test_11_eyewitness); (3) `/api/config/profiles/*` create→list→save→read-back→activate→delete round-trip (was 200-status-only in test_ui_wiring). Phase 4C (file browser modal) was a no-op — `test_selenium_project::TestProjectSaveOpen` tests 01-09 already cover the full save/open file-browser flow end-to-end. **Skill update**: `.claude/skills/legion-selenium-test/SKILL.md` got v10.188 lessons (DB-file-deletion-under-live-engine; never SIGKILL all descendants from project switch) + new "Verification design — what makes an assertion non-hollow" section with 6 rules (verify state not invocation; pre+post assertions; selectors must match what JS emits; pytest.skip not silent return; process completion via API not DOM).
- **v10.193–v10.199**: `run_tests.sh` quality sweep — retry on failure with configurable count; per-suite count-up timer next to spinner; timing estimates (actual elapsed per suite stored and shown next run); bash portability fixes: replaced bool-as-command patterns (`$?` → explicit `if`), eliminated `&& { }` compound constructs; replace misleading skip notes in test files. No production code changes.
- **v10.200**: Test fix — US09 `test_dom_percent_matches_snapshot_percent` was asserting `str(None) == ''`; when nmap finishes too fast for `--stats-every 5s` the percent column is legitimately `None`. Fixed by normalising both sides.
- **v10.201**: Ctrl+B orange flash broken in upper output panel when 2+ process tabs open — `_lastNonXtermSelSource` was not being set correctly for the upper panel's `dyn-output-*` elements when multiple tabs existed. Fixed the mousedown capture listener scope.
- **v10.202**: Tab nav bar enhancements — ◀◀ / ▶▶ first/last jump arrows added alongside existing ◀/▶ scroll arrows; centred "Tab" label added between them. CSS `?v=89`.
- **v10.203**: Tab bar scrollbar gutter — scrollbar track now always occupies space (no layout shift when scrollbar appears/disappears). `scrollbar-gutter: stable` added with explicit height.
- **v10.204**: Three scheduler/AI bugs: (1) scheduler saw 0 ports for 2nd+ hosts — `session.remove()` was called once before `getHosts()` but the per-host `getPortsAndServicesByHostIP()` used the stale cached session; fixed by adding `session.remove()` inside the per-host loop. (2) checkDuplicate layer-2 permanently blocked all scheduler tools whose service names only resolved at the NSE/vulners stage — if `existing==0` (no prior process) the layer-2 script-count check no longer blocks. (3) AI tool outputs always empty — `_assemble_host_data()` enrichment loop used `getProcessById()` which has no `output` key, overwriting the correct output from `getProcesses()` LEFT JOIN with `''`. Fixed to use `proc.get('output','')` directly. Also: `run_phase1()` now saves to `ai_history.db` (with `phase2_markdown=None`); `run_phase2()` updates the entry. `_dynOutputCache` added — caches rendered innerHTML for Finished/Killed/Crashed processes so 736KB smbmap output is not re-parsed via ansiToHtml+highlightMatches every 1.5s poll. JS `?v=107`.
- **v10.205**: Two related upper-output-panel bugs fixed by removing `outputEl.textContent = 'Loading...'` from every poll-rebuild cycle. Root cause: the dynamic tab click handler (fired every 1.5 s by `renderDynamicToolTabs` → `restoredBtn.click()`) unconditionally set `textContent='Loading...'` before calling `loadProcessOutput`. This caused two cascading problems: (1) **scroll reset** — the element was saved by `renderDynamicToolTabs` in the "Loading..." state (scrollHeight≈clientHeight → gap<40 → saved as 'bottom' or 0), permanently overwriting the user's actual scroll position and creating a loop where scroll reset to beginning on every 1.5 s cycle; (2) **intermittent loading delay** — Running processes (excluded from `_dynOutputCache`) still showed "Loading..." for the full async fetch duration (200ms–2s) on every poll. Fix: `outputEl.textContent = 'Loading...'` now only fires on a genuine first-ever load (`_procScrollPos[procId] === undefined && _dynOutputCache[procId] === undefined`). Subsequent poll-rebuilds skip the text mutation entirely — Finished processes serve from cache instantly, Running processes show a blank element during the 200ms–2s fetch with no scroll disruption. JS `?v=111`.
- **v10.206** (large batch — legion.conf tool audit + new tests): Full audit of `legion_tools.html` revealed multiple gaps. (1) **conf fixes**: `banner` command replaced `bash -c "echo | nc..."` with `nmap -Pn -p [PORT] --script banner [IP]` (bash triggered Interactive PTY — no stored output); `feroxbuster` SchedulerSettings scope removed `ssl`/`https` (http:// URL fails silently on TLS ports); `fierce-dns` fixed to `fierce --domain [IP] --dns-servers [IP] --subdomain-file ...` (was `--subdomains` with no `--domain`); `dnsrecon` PortActions fixed from `--dns-servers`/`--xml` to `-n`/`-x`; `nuclei` commands now pass `-t /home/kali/.local/nuclei-templates/http`; Hydra `-o` commands had embedded quotes removed; `dnsrecon` and `nbtscan` added to SchedulerSettings; `kerbrute-users` added to PortActions (right-click only — requires `--domain`). (2) **install.sh**: added `fierce`, `seclists` to apt block; `kerbrute` added to `_CRITICAL_TOOLS` health check; nuclei config dir created; `Encoding::BER` Perl module installed for nbtscan; fierce/seclists file-presence health checks. (3) **test_tool_installation.py** (51 tests, `--noconftest`, no server needed): conf integrity checks, binary presence checks, Perl dep checks, nuclei template check, dnsrecon flag guard (`-n` not `--dns-servers`), kerbrute scope guard (`userenum` not spray/brute). (4) **test_victim_tool_execution.py** (179 parametrized tests): spawns 127.42.0.1 loopback with socat + real services covering all service names in SchedulerSettings; `create_test_app(enable_scheduler=True)`; 3 test functions per tool: `test_every_tool_triggered`, `test_every_tool_expected_output`, `test_every_tool_no_config_error`. (5) **Easy Mode Save bug**: Save button read the raw conf textarea while Easy Mode panel was open (textarea still had old content). Fixed with a capture-phase `addEventListener('click', fn, true)` listener that serialises Easy Mode state into the textarea before the Save handler reads it.
- **v10.207**: Easy Mode Apply button highlight — `@keyframes easyApplyPulse` CSS animation + `#easy-apply-btn.easy-apply-dirty` class. `_markDirty()` adds the class on any input/change event in Easy Mode; `_clearDirty()` removes it when Apply is clicked. Reminds user to apply before closing. CSS `?v=89`.
- **v10.208**: Services tab single-port selection — clicking a service shared across multiple ports (e.g. `http` on 80 and 8080) was highlighting ALL rows with that service name on every 1.5s snapshot re-render. Added `L.selectedServicePort` to the state object; click handler stores `tr.dataset.port`; `renderServices()` check requires both `L.selectedService === s.service && L.selectedServicePort === (s.port || '')`. JS `?v=112`.
- **v10.209**: Service results stay-in-place — clicking a row in the right-panel cross-host service results (`updatePortsByService`) was calling `loadHostDetail()` + `renderHosts()`, replacing the service results view with the host's full service list. Fixed: click handler now only highlights the clicked row (`body.querySelectorAll('tr')` toggle) and silently updates `L.selectedHostId`/`L.selectedHostIp` for context-menu support. JS `?v=113`.
- **Fresh-VM audit + install hardening (post-v10.209)**: Full fresh-install run of `gcloudInstall_V5.sh` exposed multiple gaps across install.sh, test suite, and the master install script. All fixes consolidated here:
  - **install.sh**: (a) Added victim test server deps `nginx mariadb-server redis-server samba snmpd xrdp`; removed postgresql entirely — Kali already has it via `metasploit-framework` and every attempt to suppress the `pg_upgradecluster` whiptail dialog failed (skip-if-installed, `apt-mark hold` partial, `apt-mark hold` global). Only `postgresql-client` (psql CLI) remains. (b) Fixed pip install FATAL on re-run — pip output piped through grep; when all packages already satisfied, grep returns 1 (no matches) and `die` ran even though pip rc=0. Fix: check `$PIP_RC` not grep's exit code. (c) Pass `SUDO_USER="${REAL_USER}"` to step 8 verification pytest — inner `sudo` wiped env, `_real_home()` resolved to `/root` instead of `/home/kali`, nuclei template path check always failed. (d) Remediation loop deduplicates binary installs via associative array.
  - **test_victim_tool_execution.py**: (a) Module-level `pytestmark` skip guard with `_victim_prereq_reason()` — probes `127.0.0.1` (not `127.42.0.1` where mariadb/redis don't listen); fixture uses `pytest.skip()` not `RuntimeError`. (b) Constants moved above prereq function so `VICTIM_IP` is defined at import time. (c) Fixture expected nginx on 5 ports (80/443/4848/8080/8443) but default nginx only listens on 80 — remaining ports timed out and all 184 tests skipped. Fix: nginx only required on port 80; moved 443/4848/5432/8080/8443 to socat listeners. (d) Removed postgresql from real_service_ports (already on Kali via metasploit); added port 5432 to socat.
  - **test ordering**: `test_requirements` + `test_tool_installation` → positions 2–3 in unit (were 25–26). `test_victim_tool_execution` → position 1 in selenium — passing it IS the install proof.
  - **test_ui_v10e_features.py**: wig tests inverted — was asserting wig IS in scheduler/port-menu; `test_tool_installation.py` correctly asserted it is NOT (Python 3.13 crash). `wig` removed from `test_requirements.py` `KALI_APT_TOOLS`.
...
- **v10.224**: Multi-provider AI support — `analyzer.py` refactored from hardcoded Vertex AI to provider factory. New `_read_ai_config()` reads `[AISettings]` from `legion.conf` (with legacy `~/.claude/settings.json` fallback for existing Vertex users). Three providers: `vertex` (GCP ADC), `anthropic` (direct API key), `openai` (OpenAI-compatible — covers OpenAI, Google Gemini, Azure, ollama, vLLM, LM Studio via custom `ai_api_url`). `_OpenAIAdapter` wrapper translates OpenAI chat completions to Anthropic response shape. Status endpoint (`/api/ai/host/<id>/status`) now returns `ai_provider`, `ai_model`, `ai_configured`, `ai_config_error`. UI: `ai-not-configured` panel with setup instructions, provider badge in toolbar. `[AISettings]` added to `masterLegion.conf` + live conf + config manager whitelist. `openai>=1.0.0` added to `requirements.txt`. `settings.py` `getAISettings()` accessor added. JS `?v=124`.
- **v10.225**: Enhanced AI Phase 1 — automated tool enumeration with batch approval. Phase 1 now runs a 4-step pipeline: (1) initial synthesis, (2) LLM gap analysis (recommends tools from legion.conf + safe allowlist), (3) batch approval UI (user reviews all commands with real IPs, checks/unchecks each, can install missing tools inline), (4) re-synthesis with enriched data. Async job model: `POST /phase1` returns `job_id`, UI polls `GET /phase1/progress/<job_id>`, user approves via `POST /phase1/approve/<job_id>`. Command overrides: LLM can customize conf template commands (wordlists, paths, flags) with binary-match + injection-pattern validation. 19 new PortActions: AD no-cred tools (`impacket-getnpusers-nopass`, `impacket-lookupsid-null`, `rpcclient-null-enum`, `ldapsearch-anon/-users/-all-attrs`, `gpp-sysvol-check`, `gpp-extract`, `certipy-find`, `adidnsdump-enum`, `ldeep-enum`, `windapsearch-users`, `responder-finger`, `subfinder-enum`) + targeted NSE (`nmap-shellshock/-heartbleed/-smb-vuln/-ftp-anon/-http-enum`). New `scripts/python/gpp_check.py` — SYSVOL download + XML parse + gpp-decrypt in one shot. DB: `gap_analysis_json` + `enum_actions_json` columns on `ai_analysis` + `ai_sessions`. `install.sh`: gpp-decrypt, python3-impacket, responder, certipy-ad, adidnsdump, ldeep, pywerview, windapsearch, subfinder. Re-synthesis prompt flags credentials/hashes with hashcat mode numbers. Bug fix: `phase2_markdown=None` → `''` (NOT NULL constraint). JS `?v=125`.
- **Tests**: `tests/test_ui_v10g_features.py` (port 5104, 38 tests): conf integrity (7 — PortActions parse, service filters, GPP script valid), DB schema (2 — new columns round-trip), analyzer validation (9 — safe command accept/reject, override binary mismatch/injection/valid, tool catalog, known binaries), async API (5 — job_id return, progress poll, 404 handling, sync fallback), status endpoint (2 — est_cost_enhanced field), latest endpoint (1 — gap/enum JSON in response), Selenium UI (4 — AI tab exists, state on host select, enum/attacks divs), history DB (1 — save_session with new columns), regression (7 — existing run_phase1/phase2/analysis preserved, estimate_cost works, routes respond). Integrated into `run_tests.sh --selenium`.
- **v10.226–v10.239**: Post-v10.225 fixes: (1) Missing `var` declarations for `_aiJobId`/`_aiProgressIndex`/`_aiPollTimer` caused `ReferenceError` in promise callback — Analyze button appeared to do nothing (silent catch). (2) Approval panel scroll overflow — `#ai-right` changed to `overflow-y:auto`, inner findings table capped at `40vh`, wider brighter scrollbars (16px, `#999` thumb). (3) Step timeline — replaced single overwritten label with accumulating vertical timeline (✓ past / ⟳ current / ○ future). (4) Approval buttons pinned to bottom — rendered as direct child of `#ai-right` with `flex-shrink:0`, removed on submit. (5) Multi-host AI jobs — `_aiJobs[hostId]` dict stores per-host job state; switching hosts saves/restores polling + runDiv HTML. (6) Tab memory on host switch — `L._hostActiveTab[hostId]` saves last active right-panel tab; restored on return instead of always forcing Services. (7) Provider badge always visible — fetches `/api/ai/host/*/status` on every AI tab open regardless of analysis state. (8) AI process tab titles — `AI: tool_id (port/protocol)` convention; safe commands include path (`AI: curl (80/tcp/robots.txt)`) or search term (`AI: searchsploit (apache 2.4)`). (9) Partial DB save after Step 2 — `ai_analysis` + `ai_sessions` rows written after synthesis + gap analysis so results survive if user leaves during Step 3; Step 4 UPDATEs the same rows. (10) Phase 2 gap context priority — `run_phase2()` includes `gap_analysis_json` in prompt with CONFIRMED BY TARGETED ENUMERATION section; system prompt prioritizes gap-fill findings. (11) Cost dropdown restored — comparison dropdown in results panel now shows cost. (12) `--open FILE` CLI flag — `legion.py --web --open /path/to/saved.legion` pre-loads project before server starts. (13) Approved commands stay visible during Step 3 — checkboxes disabled, unapproved faded, inline status spans (`⟳ queued` → `✓ Finished`) update dynamically every 2s via `pid_to_proposed` map. (14) Export HTML fixed — `_aiResults.phase2_markdown` now updated when Phase 2 completes; export includes Automated Enumeration section with commands + obvious attacks. JS `?v=135`, CSS `?v=94`.
- **v10.250**: Nuclei full template coverage — replaced 2 monolithic nuclei entries (10,727 HTTP templates in one process that often timed out) with 40 per-category scans across 6 template categories: 34 HTTP (17 subdirs × 2 http/https), plus network (281 TCP protocol templates), ssl (38 TLS checks), dast (249 XSS/SQLi/SSTI with `-dast` flag), javascript (112 default-login checks with `-code` flag), and dns (30 templates, right-click only). Each category runs as a separate Legion process. `nuclei-tokens` removed (self-contained API key tests, not port scanners). All 8 conf files updated.
- **v10.253**: 7 broken tool configs fixed: polenum (piped empty password — was hanging on interactive prompt), samrdump (`''@[IP] -port [PORT]` — was passing `[PORT]/SMB` as invalid positional arg), searchsploit-nmap (`bash -c` → `sh -c` — bash triggered PTY mode), smbmap 4 entries (`--no-pass` removed — was Kerberos CCache flag not null session; `-R` → `-r`; added `--no-banner`), gobuster-dns removed from scheduler (requires domain name not IP).
- **v10.254**: `[low]` and `[medium]` match highlighting broken by ANSI color spans — nuclei wraps severity in ANSI codes which `ansiToHtml()` converts to `<span>` tags, breaking the plain substring regex in `highlightMatches()`. Fix: insert `(?:</?span[^>]*>)*` between each regex-escaped token so patterns match regardless of ANSI span wrapping. JS `?v=137`.
- **v10.255**: `[critical]` added to global-positive match keywords. AISettings section added to Easy Mode config editor (`FORM_DEFS` + `SECTIONS` array) — ai_provider dropdown, api_key, model, api_url, vertex_project_id, vertex_region. JS `?v=138`.
- **v10.256**: 4 test fixes: test_save_open_data match keyword changed from removed `tcp open` to stable `200 OK`; test_escape_hides_find_bar updated for v10.221 always-visible find bar (assert input cleared not bar hidden); test_phase4_state H1.1 scan window 1000→2500 chars; waybackurls test uses `--help` instead of pipe.
- **v10.257**: Scan speed profiles — 3 shipped profiles controlling scheduler auto-triggers: `scan-fast.conf` (74 tools — screenshots, CVE checks, default creds, key enum, one dir bruter), `scan-medium.conf` (120 tools — adds multiple dir bruters, most nuclei categories, web scanners, deeper AD/LDAP/SMB), `scan-extensive.conf` (179 tools — everything, identical to masterLegion.conf). All share identical non-SchedulerSettings sections. Auto-copied to user profiles dir via `_ensure_profiles()`. 12 new tests: files exist, subset chain (fast ⊂ medium ⊂ extensive), extensive = master, non-scheduler sections identical.
