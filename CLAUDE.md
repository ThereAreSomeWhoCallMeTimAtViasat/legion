...
- **Fresh-VM audit + install hardening (post-v10.209)**: Full fresh-install run of `gcloudInstall_V5.sh` exposed multiple gaps across install.sh, test suite, and the master install script. All fixes consolidated here:
  ...
  - **test_victim_tool_execution.py**: (a) Module-level `pytestmark` skip guard with `_victim_prereq_reason()` — probes `127.0.0.1` (not `127.42.0.1` where mariadb/redis don't listen); fixture uses `pytest.skip()` not `RuntimeError`. (b) Constants moved above prereq function so `VICTIM_IP` is defined at import time.
