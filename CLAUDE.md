...
- **Fresh-VM audit + install hardening (post-v10.209)**: Full fresh-install run of `gcloudInstall_V4.sh` exposed six gaps fixed across multiple commits: (1) **test ordering** — `test_requirements.py` + `test_tool_installation.py` moved to positions 2–3 in unit section (were 25–26); `test_victim_tool_execution.py` moved to position 1 in selenium section — passing it IS proof the install is good. (2) **victim test skip guard** — `_victim_prereq_reason()` now socket-probes ports after starting services (was checking `systemctl list-unit-files` = installed, not running); fixture uses `pytest.skip()` instead of letting `RuntimeError` propagate as an opaque "0 of 0" error. (3) **wig test conflict** — `test_ui_v10e_features.py` asserted wig IS in scheduler/port-menu; `test_tool_installation.py` correctly asserted it is NOT (Python 3.13 crash). Selenium test inverted to match reality. `wig` removed from `test_requirements.py` `KALI_APT_TOOLS`. (4) **install.sh victim server deps** — added `nginx mariadb-server redis-server postgresql samba snmpd xrdp` (server packages, not just clients); made install visible (removed `2>/dev/null || true`); health check uses `dpkg -l` for server packages since their binaries are in `/usr/sbin`. (5) **postgresql cluster-upgrade blocker** — `pg_upgradecluster` calls `whiptail` directly, bypassing debconf; `DEBIAN_FRONTEND=noninteractive` has no effect. Fix: skip `postgresql` install entirely if any version is already present — victim test only needs a postgres server on 5432, any version. (6) **run_tests.sh all-skipped parsing** — when 100% of tests skip (victim test on machine without services), pytest exits rc=0 with `184 skipped in 2.15s`; grep pattern `passed|failed|error` missed "skipped"-only line, treating it as a failure and retrying 3×. Added `skipped` to all four grep patterns. (7) **install.sh remediation loop** — same binary failing in two test files (ssh-audit in test_requirements + test_tool_installation) triggered `apt-get install` 3× instead of 1×. Added dedup via associative array.
- **gcloudInstall_V5.sh**: New master VM setup script replacing V4. Fixes: (1) `cd ~/Downloads` as root → `/root/Downloads` (doesn't exist) — now uses `$SUDO_USER` to find real user home; (2) `claude-auth.sh` uses macOS `open -na` — bypassed, calls `gcloud auth login --update-adc` directly with a wrapper script that opens Firefox in background (`firefox ... & exit 0`) so gcloud doesn't wait for the window to close; (3) `setup-claude-code.sh` run as root → claude installs to `/root/` — now runs via `sudo -u kali`; (4) `tar xzvf` extracts to `/root` — now extracts as kali user into `$REAL_HOME`; (5) hardcoded tar filename — now searches `claude-data*.tar.gz`; (6) interactive prompts in setup-claude-code.sh — piped `y\ny\n` answers; (7) legion repo owned by root after clone — `chown -R kali:kali`.
- **postgresql removed from install.sh** — Three attempts to suppress the `pg_upgradecluster` whiptail dialog all failed: (1) skip if installed — apt upgrades it when installing other packages, (2) `apt-mark hold` in one block — 20+ other apt-get calls after the block, (3) hold at top of script — glob patterns don't cover all package names. Correct fix: don't install it at all. postgresql is already present on Kali via the `metasploit-framework` dependency. Only `postgresql-client` (the `psql` CLI) remains in install.sh; it installs no server and triggers no cluster upgrades. The victim test uses whatever postgresql is already on the system.
- **pip install FATAL on re-run** — The pip output was piped through `grep "Collecting|Installing|error"`. When all packages are already satisfied, pip prints only `Requirement already satisfied...` which doesn't match — grep returns exit code 1 and install.sh calls `die`. Fix: capture pip's exit code separately (`PIP_RC=$?`) and check that for success, use grep only for display filtering.
- **nuclei template path wrong in verification** — install.sh step 8 runs `sudo "${VENV_PY}" -m pytest` which resets the environment and loses `SUDO_USER`. The test's `_real_home()` falls back to `Path.home()` = `/root`, but templates are at `/home/kali/.local/nuclei-templates/http`. Fix: `sudo SUDO_USER="${REAL_USER}" "${VENV_PY}" -m pytest` preserves the real user.
- **victim test prereq probed wrong address** — The prereq check socket-probed `VICTIM_IP` (`127.42.0.1`) but mariadb and redis bind to `127.0.0.1` only (not `0.0.0.0`). Connections were refused even though services were running. All 184 tests skipped on every run. Fix: probe `127.0.0.1` in prereq (confirms services exist); the fixture handles the `127.42.0.1` loopback alias and service routing.
