...
- **postgresql cluster-upgrade final fix** — Previous fix (skip if already installed) still failed because apt's dependency resolver can upgrade postgresql when installing ANY unrelated package (nginx, samba, xrdp, etc). There are 20+ `apt-get install` calls throughout install.sh; the partial hold only covered one block. Fix: `apt-mark hold postgresql postgresql-common "postgresql-1[0-9]"` at line 184 (the very first step, before any apt-get call) and `apt-mark unhold` at the very last line. postgresql cannot be upgraded at any point during install.sh regardless of what triggers it.
