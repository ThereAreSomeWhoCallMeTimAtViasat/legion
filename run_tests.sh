...
for f in "${UNIT_FILES[@]}"; do
    echo -ne "  running ${f##tests/} ..."
    run_unit "$f"
    # Reprint the last summary line so progress is visible
...
if [[ -z "$LIVE_TARGET" ]]; then
    record "test_terminal T7 (live)" "skip" "LEGION_TEST_TARGET not set — skipped"
    echo -e "  ${SUMMARY_LINES[-1]}"
else
    echo -ne "  running test_terminal T7 live (target: $LIVE_TARGET) ..."
    out=$(sudo env LEGION_TEST_TARGET="$LIVE_TARGET" python3 tests/test_terminal.py 2>&1) || true
    results_line=$(echo "$out" | grep "^Results:" | tail -1)
    passed=$(echo "$results_line" | grep -oP '\d+(?= passed)' || echo 0)
    failed=$(echo  "$results_line" | grep -oP '\d+(?= failed)' || echo 0)
    skipped=$(echo "$results_line" | grep -oP '\d+(?= skipped)' || echo 0)
    detail="${passed}p ${failed}f ${skipped}s"
    if [[ "$failed" -eq 0 ]]; then
        record "test_terminal T7 (live)" "pass" "$detail"
    else
        record "test_terminal T7 (live)" "fail" "$detail"
    fi
    echo -e "\r  ${SUMMARY_LINES[-1]}"
fi
