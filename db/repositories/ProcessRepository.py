    ...
    def getProcesses(self, filters, showProcesses: Union[str, bool] = 'noNmap', sort: str = 'desc', ncol: str = 'id'):
        ...
        if showProcesses == 'noNmap':
            query = text(
                'SELECT '
                '0 AS progress, '
                'CASE WHEN SUM(CASE WHEN process.display = "True" THEN 1 ELSE 0 END) > 0 THEN "True" ELSE "False" END AS display, '
                'COALESCE(MAX(process.elapsed), 0) AS elapsed, '
                'COALESCE(MAX(process.percent), "") AS percent, '
                '"" AS pid, '
                'COALESCE(process.name, "") AS name, '
                'COALESCE(MAX(process.tabTitle), "") AS tabTitle, '
                '"" AS hostIp, '
                '"" AS port, '
                '"" AS protocol, '
                '"" AS command, '
                'COALESCE(MAX(process.startTime), "") AS startTime, '
                'COALESCE(MAX(process.endTime), "") AS endTime, '
                '"" AS outputfile, '
                '"" AS output, '
                'COALESCE(MAX(process.status), "") AS status, '
                '"False" AS closed, '
                'MAX(process.id) AS id '
                'FROM process AS process '
                'WHERE process.closed = "False" AND process.display = "True" '
                'GROUP BY process.name '
                'ORDER BY MAX(process.id) DESC'
            )
